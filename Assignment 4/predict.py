...
class SentiInference:
    def __init__(self, arg_file="./results/senti_cls_bilstm.args", model_file="./results/senti_cls_bilstm.m"):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.args, self.model = self.load_model(arg_file, model_file)

    def load_model(self, arg_file="./results/senti_cls_bilstm.args", model_file="./results/senti_cls_bilstm.m"):
        """
        Inputs:
    ...
    def predict(self, doc="", topk=5):
        ...
        doc_tensor = Data2tensor.idx2tensor([doc_ids], self.device)
        doc_lengths_tensor = Data2tensor.idx2tensor([len(doc_ids)], self.device)
        output, hidden , hidden1 = self.model.forward(doc_tensor, doc_lengths_tensor)
        prob, pred = self.model.inference(output, topk)
        pred_lb = list()
        pred_probs = list()
        for index in range(topk):
            pred_lb.append(self.args.vocab.i2l[pred[0][index].item()])
            pred_probs.append(prob[0][index].item())
        # YOUR CODE ENDS HERE
        #######################
