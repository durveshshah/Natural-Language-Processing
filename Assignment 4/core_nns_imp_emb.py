...
class RNNModule(nn.Module):
    ...
    def __init__(self, rec_type, ntokens, emb_size, hidden_size,vocab, nlayers, dropout=0.5, bidirect=False):
        ...
        self.glove_embedding(vocab)

    # method to initialize glove embedding
    def glove_embedding(self,vocab):

        # Getting vocab files and words
        list_to_append = []
        for i in vocab:
            list_to_append.append(vocab[i])

        # Converting words to vectors
        model = Word2Vec([list_to_append], min_count=1)

        # Saving model files
        model.wv.save_word2vec_format('word2vec.txt', binary=False)

        # Converting to glove to vectors
        output_file_path = './word2vec.txt'

        # Loading glove vector file
        model = KeyedVectors.load_word2vec_format(output_file_path)

        # attach weights to embedding layer
        self.embedding_layer.weights = torch.FloatTensor(model.vectors)
...
class UniLSTMModel(RNNModule):
    ...
    def forward(self, input_tensor, input_lengths=None, init_hidden=None):
        ...
        extract_last_hidden = self.get_last_hidden(rec_hidden)

        # Dropout on the last layer
        dropping_last_layer = self.dropout_layer(extract_last_hidden)

        # Calculate un-nomalized scores
        decoded_scores = self.scorer_layer(dropping_last_layer)
    ...
    def NLL_loss(self, label_score, label_tensor):
        ...
        # YOUR CODE STARTS HERE
        batch_loss = None
        batch_loss = self.celoss_layer(label_score, label_tensor)
        # YOUR CODE ENDS HERE
        #######################
        return batch_loss
    ...
    def inference(self, label_score, k=1):
        ...
        soft = F.softmax(label_score, dim=-1)
        label_prob, label_pred = soft.data.topk(k)
...
class BiLSTMModel(UniLSTMModel):
    ...
    def forward(self, input_tensor, input_lengths=None, init_hidden=None):
        ...
        # sort lengths of input tensors in the descending mode
        tensor, input_lengths, ten1, ten2 = self.sort_tensors(input_tensor, input_lengths)

        # Project word embedding vectors [e1,...,en] from index input_tensor [id_w1,..., id_wn]
        embedding_layer = self.embedding_layer(tensor)

        # Dropout on embedding
        drop_emb_layer = self.dropout_layer(embedding_layer)

        # Pass embedding vectors [e1,...,en]  into a recurrent model to learn hidden states [h1, ..., hn]
        rec_output, rec_hidden = self.get_all_hidden(drop_emb_layer, input_lengths, init_hidden)

        # recover the original order of outputs to compute loss
        rec_output = self.resort_tensors(rec_output, ten2)
        rec_hidden = self.resort_tensors(rec_hidden, ten2)

        # Extract the last hidden vector
        get_last_hidden = self.get_last_hidden(rec_hidden, True)

        # Learn the hidden representation
        learning_representation = self.fc_layer(get_last_hidden)

        # Dropout on the last layer
        dropping_last_layer = self.dropout_layer(learning_representation)
