...
from utils.core_nns_imp_emb import UniLSTMModel, BiLSTMModel
...
class Sentimentmodel(object):
    def __init__(self, args):
        ...
        if args.bidirect:
            self.model = BiLSTMModel(args.model, self.ntokens, args.emb_size, args.hidden_size,args.vocab.i2w, args.nlayers,
                                     args.dropout,
                                     args.bidirect, nlabels=self.nlabels).to(self.device)
        else:
            self.model = UniLSTMModel(args.model, self.ntokens, args.emb_size, args.hidden_size,args.vocab.i2w, args.nlayers,
                                      args.dropout,
                                      args.bidirect, nlabels=self.nlabels).to(self.device)
    ...
    def evaluate_batch(self, eval_data):
        ...
        with torch.no_grad():
            for doc_batch, lb_batch in eval_batch:
                ...
                initial_tensor = Data2tensor.idx2tensor(doc_pad_ids, self.device)
                length_tensor = Data2tensor.idx2tensor(doc_lengths, self.device)
                label_score = Data2tensor.idx2tensor(lb_batch, self.device)
                output, _, _ = self.model(initial_tensor, length_tensor)
                total_docs += initial_tensor.size(0)
                total_loss = self.model.NLL_loss(output, label_score)
                y_prob, tmp_y_pred = self.model.inference(output)
                y_true.extend(label_score.tolist())
    ...
    def train(self):
        ...
        try:
            ...
            for epoch in range(1, self.args.epochs + 1):
                ...
                if not best_val_f1 or val_metrics["f1"] < best_val_f1:
                    nimp = 0
                    best_epoch = epoch
